...
class URDualArmTask1Env(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.xy_randomize:
            ...
            # Use the full (table_half - margin) range.
            max_off = avail
        ...
        # With MuJoCo qpos order [w, x, y, z], the resulting quaternion is:
    ...
    def _compute_reward_done(self) -> Tuple[float, bool]:
        ...
        # Increase orientation shaping so the policy learns 6DOF alignment
        ori_reward = -0.4 * np.arctan(ori_angle)

        # Collision penalty: if object touches table, discourage dropping it
    ...
    def step(self, action):
        ...
        # Apply ctrl to actuators in model order
        for i, aid in enumerate(self.actuator_ids):
            self.data.ctrl[aid] = float(ctrl[i])

        # Simulate multiple internal steps for stability
        unstable = False
        ...
        # Terminal bonus once streak threshold is reached.
        if done:
            reward += 20.0
...
def main():
    ...
    parser.add_argument("--log_dir", type=str, default="./logs/ppo_ur_task1/")
    parser.add_argument("--model_path", type=str, default="./models/piper_ur_task1_ppo_model.zip")
